...
hub_yaml = {
  ...
  'assertion_consumer_service_uri' => 'http://dev-hub.local/SAML2/SSO/Response/POST',
...
Dir::chdir(output_dir) do
  ...
  idps.each do |name, id|
    yaml = idp_yaml.update(
      'organization' => { 'name' => id, 'url' => 'http://localhost', 'display_name' => name},
      'entity_id' => "http://stub_idp.acme.org/#{id}",
      'sso_uri' => "http://stub_idp.acme.org/#{id}/SAML2/SSO",
      'id' => id
    )
